...
| # | 機能 | 区分 | 現状 | 残作業 |
|---|---|---|---|---|
| F-1 | 顧客一覧＝受信トレイ（最新のやりとり順・未対応が上・会社/店舗列） | MUST | **済**（2026-09-06 店舗フィルタ追加） | — |
| F-2 | 会社/店舗の切替（スタッフ） | MUST | **済**（全社→会社→店舗の3段階） | — |
| F-3 | ステータス（会社別・横断カテゴリ） | MUST | **済**（2026-09-08。最後の NEW は削除・カテゴリ変更で消せない。顧客が使っているステータスの削除も件数を添えて止める） | — |
| F-4 | タグ（顧客タグ） | MUST | **済**（2026-09-06） | 一覧の絞り込み・詳細で付け外し・自動付与（店舗振り分け／A層「意欲あり」／アポ確定）。候補は `Organization.tagPresets` |
| F-5 | 反響元（sourcePortal）と反響内容・物件 | MUST | あり（SUUMO/アパマン/HOME'S パーサ） | 会社が使うポータル別パーサの追加（アットホーム・スモッカ・エイブル系） |
| F-6 | 会話タイムライン（メール/LINE/SMS/架電/メモ統合） | MUST | **済**（2026-09-11。添付を各メッセージに表示し、認証付き経路から取得） | 受信添付の実機確認（実際に添付付きの反響を受けるまで未検証） |
| F-7 | 統合コンポーザー（定型文・変数・SMS分割・NG表現警告） | MUST | **済**（2026-09-11。メールに添付を付けて送信） | LINE 画像（D-2） |
| F-8 | 下書き承認（承認待ち一覧・承認/却下） | MUST | **済**（2026-09-08。却下は `REJECTED`＋理由・人・日時を保存。送信失敗 `FAILED` と区別） | — |
...
| F-15 | 来店セルフ予約（連動なしは未確定→担当者の連絡で確定） | SHOULD | **済**（2026-09-08 PR #43） | — |
| F-21 | 店舗スケジュール連動（空き照会・予定書き込み・即時確定） | SHOULD（連動する店舗のみ MUST） | **INTERNAL のみ済**（2026-09-08 PR #43） | 外部カレンダー（Google→サイボウズ→TimeTree 可否確認）の接続。未接続の provider は「連動なし」として扱う |
| F-16 | レポート（反響数・初回返信までの時間・アポ率・会社別） | SHOULD | `/analytics` 骨組み | 定義（§4.1）に合わせて再実装 |
| F-17 | 検索（氏名・電話・メール・物件名） | MUST | **済**（2026-09-06。受信トレイの検索窓。電話はハイフン有無どちらでも引ける） | `/customers` 一覧側は未 |
| F-18 | 会社ごとの LINE 公式・送信ドメイン | MUST（2社目以降） | **済**（2026-09-08 PR #42） | 各社の LINE 公式・送信ドメインの実登録（運用作業） |
...
- **2026-09-08 D-1 第3弾（PR #43）**: M-6 承認制セルフ予約 と M-10 店舗スケジュール連動（INTERNAL）。
  ...
  - テスト 68件 → **83件**
- **2026-09-08 D-1 第4弾（PR #44）**: F-3 と F-8。
  - F-3: 会社から「新規(NEW)」カテゴリのステータスが消える操作（最後の1件の削除・カテゴリ変更）を止めた。消えると新規反響が受信トレイの未対応にも横断集計にも出なくなる。あわせて、顧客が使っているステータスの削除を「◯件あります」と件数付きで止めた（従来は外部キー違反で 500 になっていた）
  - F-8: 下書きの却下を `FAILED`（送信失敗）ではなく `REJECTED` にし、`rejectReason` / `rejectedByUserId` / `rejectedAt` を保存。却下時に理由を尋ねる
  - テスト 83件 → **91件**
- **2026-09-11 D-1 第5弾（PR #45）**: F-6 / F-7 添付ファイル。
  - `Attachment` テーブル。実体は Vercel Blob、**`blobUrl` は API のレスポンスにも画面にも出さない**。取得は `/api/attachments/[id]/download` だけで、ログイン＋所属会社を確認し、誰がいつ取ったかを監査ログに残す（顧客の本人確認書類が、推測されにくいだけの公開URLで社外に出るのを避ける）
  - 受け入れ判定は `lib/attachment-rules.ts`（DB にも Blob にも触らない純粋な関数）。**許可リスト方式**の Content-Type、実行ファイルの拡張子は Content-Type を偽っても拒否、1ファイル 10MB・1通 20MB・10件まで、ファイル名はパス遡りを潰して正規化
  - 送信は添付の実体を取り出せなかった時点で中止する（添付が抜けたまま「送れました」にしない）
  - 受信メールの添付は `lib/inbound-attachments.ts` で取り込む（base64・URL の両形式に対応）。**実機未検証**
...
- v1.11 2026-09-08: D-1 第4弾。F-3（NEW ステータスの保護）・F-8（却下理由の保存）を実装（PR #44）。§4 の表を実装に合わせて更新。
- v1.12 2026-09-11: D-1 第5弾。F-6 / F-7 添付ファイルを実装（PR #45）。実体は Vercel Blob、配信は認証付き経路のみ。
