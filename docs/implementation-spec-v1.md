...
| 個人情報の保持 | 最終接触から2年で匿名化（氏名・連絡先・本文を不可逆にマスク、統計用の日付・反響元は残す） | **提案（§9）** |
| 削除依頼 | 受領から7営業日以内に匿名化 | **提案（§9）** |
...
| 権限 | §3 の表を **2社＋staff の3ユーザー**で総当たり（自社=200、他社=403、未ログイン=401/307） | テスト用 Organization 2社を seed し、API を直接叩く統合テスト | **なし（D-0 で作る）** |
| 監査 | 主要操作後に AuditLog が1行以上 | 同上 | なし |
| E2E | 反響受信→下書き→承認→送信→受信→分類→アポ確定 | 本番と同構成の staging（Neon ブランチ）で手動 | staging なし |
...
| # | 論点 | 選択肢 | 推奨 |
|---|---|---|---|
| Q-1 | 個人情報の保持期間 | A: 最終接触から2年で匿名化 / B: 3年 / C: 期限なし（削除依頼のみ） | A（宅建業法の帳簿5年は取引成立分のみ。未成約の反響は2年で十分） |
| Q-2 | 会社ごとの LINE 公式 | A: 各社が自社の LINE 公式を持ち込む / B: ヘヤクレス名義の LINE を会社別に作る | A（既にカナリー運用で各社持ち込みが前提） |
| Q-3 | Organization.slug の値 | 会社の英字コード（例 flat-agency） | マニュアルBの会社コードと同じ値にして二重管理を避ける |
| Q-4 | staff 付与の権限 | A: 管理者のみ / B: 管理者＋ Itaru の承認 | A（監査ログで追えるようにする） |
| Q-5 | セルフ予約の承認 SLA | 30分（S-FIT）/ 60分 | 30分。超過は Slack 通知 |
| Q-6 | staging 環境 | Neon ブランチ＋Vercel Preview（Preview の DB 接続文字列問題を解消する必要あり） | 作る（D-0） |
