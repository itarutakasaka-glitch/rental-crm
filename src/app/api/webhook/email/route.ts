...
function parseSuumo(text: string) {
  ...
  // \u540D\u524D = 蜷榊燕, \u6F22\u5B57 = 貍｢蟄・ \u30AB\u30CA = 繧ｫ繝・  const name = (text.match(/\u540D\u524D[\(・・\u6F22\u5B57[\)・云[・・]\s*(.+)/) || text.match(/\u540D\u524D\(\u6F22\u5B57\)[・・]\s*(.+)/) || [])[1]?.trim() || "";
  const nameKana = (text.match(/\u540D\u524D[\(・・\u30AB\u30CA[\)・云[・・]\s*(.+)/) || text.match(/\u540D\u524D\(\u30AB\u30CA\)[・・]\s*(.+)/) || [])[1]?.trim() || "";
  // \u30E1\u30FC\u30EB\u30A2\u30C9\u30EC\u30B9 = 繝｡繝ｼ繝ｫ繧｢繝峨Ξ繧ｹ
  const email = (text.match(/\u30E1\u30FC\u30EB\u30A2\u30C9\u30EC\u30B9[・・]\s*(\S+)/) || [])[1]?.trim() || "";
  // TEL or \uFF34\uFF25\uFF2C = ・ｴ・･・ｬ, \u96FB\u8A71\u756A\u53F7 = 髮ｻ隧ｱ逡ｪ蜿ｷ
  const phone = (text.match(/[\uFF34T][\uFF25E][\uFF2CL][・・]\s*(\S+)/) || text.match(/\u96FB\u8A71\u756A\u53F7[・・]\s*(\S+)/) || [])[1]?.trim() || "";
  // \u7269\u4EF6\u540D = 迚ｩ莉ｶ蜷・  const propertyName = (text.match(/\u7269\u4EF6\u540D[・・]\s*(.+)/) || [])[1]?.trim() || "";
  // \u7269\u4EF6\u8A73\u7D30\u753B\u9762 = 迚ｩ莉ｶ隧ｳ邏ｰ逕ｻ髱｢
  ...
  // \u6700\u5BC4\u308A\u99C5 = 譛蟇・ｊ鬧・ \u6700\u5BC4\u99C5 = 譛蟇・ｧ・  const station = (text.match(/\u6700\u5BC4\u308A\u99C5[・・]\s*(.+)/) || text.match(/\u6700\u5BC4\u99C5[・・]\s*(.+)/) || [])[1]?.trim() || "";
  // \u6240\u5728\u5730 = 謇蝨ｨ蝨ｰ
  ...
  // \u5C02\u6709\u9762\u7A4D = 蟆よ怏髱｢遨・  const area = (text.match(/\u5C02\u6709\u9762\u7A4D[・・]\s*(.+)/) || [])[1]?.trim() || "";
...
function parseApamanshop(text: string) {
  // \u30A2\u30D1\u30DE\u30F3\u30B7\u30E7\u30C3\u30D7 = 繧｢繝代・繝ｳ繧ｷ繝ｧ繝・・
  if (!/\u30A2\u30D1\u30DE\u30F3\u30B7\u30E7\u30C3\u30D7|apamanshop/i.test(text)) return null;
  // \u3010\u540D\u524D\u3011 = 縲仙錐蜑阪・ \u3010\u304A\u540D\u524D\u3011 = 縲舌♀蜷榊燕縲・  const name = (text.match(/\u3010\u540D\u524D\u3011\s*(.+)/) || text.match(/\u3010\u304A\u540D\u524D\u3011\s*(.+)/) || [])[1]?.trim() || "";
  const nameKana = (text.match(/\u3010\u540D\u524D\u30AB\u30CA\u3011\s*(.+)/) || [])[1]?.trim() || "";
  // \u3010\u30E1\u30FC\u30EB\u30A2\u30C9\u30EC\u30B9\u3011 = 縲舌Γ繝ｼ繝ｫ繧｢繝峨Ξ繧ｹ縲・  const email = (text.match(/\u3010\u30E1\u30FC\u30EB\u30A2\u30C9\u30EC\u30B9\u3011\s*(\S+)/) || [])[1]?.trim() || "";
  // \u3010\u96FB\u8A71\u756A\u53F7\u3011 = 縲宣崕隧ｱ逡ｪ蜿ｷ縲・  const phone = (text.match(/\u3010\u96FB\u8A71\u756A\u53F7\u3011\s*(.+)/) || [])[1]?.trim() || "";
  // \u3010\u304A\u554F\u3044\u5408\u308F\u305B\u5185\u5BB9\u3011 = 縲舌♀蝠上＞蜷医ｏ縺帛・螳ｹ縲・  const inquiryContent = (text.match(/\u3010\u304A\u554F\u3044\u5408\u308F\u305B\u5185\u5BB9\u3011\s*([\s\S]*?)(?:\u3010|$)/) || [])[1]?.trim() || "";
  // \u3014\u7269 \u4EF6 \u540D\u3015 = 縲皮黄 莉ｶ 蜷阪・ \u3010\u7269\u4EF6\u540D\u3011 = 縲千黄莉ｶ蜷阪・  const propertyName = (text.match(/\u3014\u7269\s*\u4EF6\s*\u540D\u3015\s*(.+)/) || text.match(/\u3010\u7269\u4EF6\u540D\u3011\s*(.+)/) || [])[1]?.trim() || "";
