...
export async function getAnalytics(organizationId: string): Promise<AnalyticsData> {
  ...
  const portalMap: Record<string,number> = {}; customers.forEach((c: any) => { const p = c.sourcePortal || "unknown"; portalMap[p] = (portalMap[p] || 0) + 1; });
  ...
  customers.forEach((c: any) => { const n=c.assignee?.name || "unassigned"; if(!assigneeMap[n])assigneeMap[n]={name:n,total:0,needAction:0}; assigneeMap[n].total++; if(c.isNeedAction)assigneeMap[n].needAction++; });
