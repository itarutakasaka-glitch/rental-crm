...
const CH = { EMAIL: { icon: "笨会ｸ・, color: "#3b82f6" }, LINE: { icon: "町", color: "#06c755" }, SMS: { icon: "導", color: "#f59e0b" }, CALL: { icon: "到", color: "#8b5cf6" }, NOTE: { icon: "統", color: "#6b7280" } };
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-4 py-3 border-b bg-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push("/customers")} className="text-gray-400">竊・/button>
            ...
            <div>
              <div className="font-bold flex items-center gap-1.5">{c.name}
                {c.isNeedAction && <span className="text-[10px] px-1.5 py-0.5 bg-red-50 text-red-500 rounded font-semibold">隕∝ｯｾ蠢・/span>}
              </div>
              <div className="text-xs text-gray-400">{c.email} ﾂｷ {c.phone}</div>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={() => start(() => { toggleNeedAction(c.id); router.refresh(); })} className="text-xs px-2 py-1 border rounded-md text-gray-500">
              {c.isNeedAction ? "閥 隕∝ｯｾ蠢・ : "蟇ｾ蠢應ｸ崎ｦ・}
            </button>
          </div>
        </div>
        {/* Messages */}
              ...
              <div key={m.id} className={`flex ${m.direction === "OUTBOUND" ? "justify-end" : "justify-start"}`}>
                <div className="max-w-[70%]">
                  <div className="text-[10px] text-gray-400 mb-0.5" style={{ textAlign: m.direction === "OUTBOUND" ? "right" : "left" }}>
                    <span style={{ color: chInfo.color }}>{chInfo.icon}</span> {m.subject && `陶 ${m.subject}`}
                  </div>
                  ...
                  <div className="text-[10px] text-gray-400 mt-0.5" style={{ textAlign: m.direction === "OUTBOUND" ? "right" : "left" }}>
                    {new Date(m.createdAt).toLocaleString("ja-JP")} ﾂｷ {m.sender?.name || c.name}
                    {m.direction === "OUTBOUND" && m.channel === "EMAIL" && (
                      m.openedAt ? <span className="text-blue-500 ml-1 font-semibold">笨凪恣 譌｢隱ｭ ({m.openCount}蝗・</span> : <span className="ml-1">笨・譛ｪ髢句ｰ・/span>
                    )}
                  </div>
                </div>
              </div>
        ...
        <div className="border-t bg-white p-3">
          ...
          {ch === "EMAIL" && <input value={subj} onChange={e => setSubj(e.target.value)} placeholder="莉ｶ蜷・ className="w-full px-3 py-1.5 border rounded-lg text-sm mb-2" />}
          <div className="flex gap-2">
            <textarea value={body} onChange={e => setBody(e.target.value)} placeholder="繝｡繝・そ繝ｼ繧ｸ..." rows={3} className="flex-1 px-3 py-2 border rounded-lg text-sm resize-none" />
            <button onClick={send} disabled={!body.trim() || isPending} className="self-end px-5 py-2 bg-primary text-white rounded-lg text-sm font-semibold disabled:opacity-40">{isPending ? "..." : "騾∽ｿ｡"}</button>
          </div>
        </div>
      </div>
      {/* Right panel */}
      <div className="w-72 flex-shrink-0 overflow-auto bg-white border-l p-4">
        <h3 className="text-sm font-bold mb-3">蝓ｺ譛ｬ諠・ｱ</h3>
        {[["豌丞錐", c.name], ["繝｡繝ｼ繝ｫ", c.email], ["髮ｻ隧ｱ", c.phone], ["蜿埼涸蜈・, c.sourcePortal], ["諡・ｽ・, c.assignee?.name]].map(([l, v]) => (
          <div key={l as string} className="flex py-1 border-b border-gray-50 text-xs"><span className="w-14 text-gray-400 flex-shrink-0">{l}</span><span>{v || "窶・}</span></div>
        ))}
        {c.properties?.[0] && (
          <div className="mt-4">
            <h3 className="text-sm font-bold mb-2">迚ｩ莉ｶ諠・ｱ</h3>
            <div className="p-2.5 border rounded-lg text-xs">
              <div className="font-semibold mb-1">{c.properties[0].name}</div>
              {c.properties[0].address && <div className="text-gray-500">桃 {c.properties[0].address}</div>}
              {c.properties[0].rent && <div className="text-gray-500">腸 {c.properties[0].rent.toLocaleString()}蜀・/div>}
            </div>
          </div>
        )}
        {c.tags?.length > 0 && (
