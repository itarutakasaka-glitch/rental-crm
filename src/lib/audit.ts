...
//   customer.preference.update / schedule.create|update|delete / line.link / message.send /
//   organization.update / template.create|update|delete / workflow.create|update
const MAX_VALUE = 500;
const trunc = (v: unknown) => (v == null ? undefined : String(v).slice(0, MAX_VALUE));

export async function logAudit(data: {
  customerId?: string | null; userId?: string | null; action: string;
...
export async function logFieldChanges(base: { customerId?: string | null; userId?: string | null; action: string },
